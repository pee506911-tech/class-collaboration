...
## Repository Notes (ClassColab)

- Monorepo: Next.js App Router in `apps/web`, Rust (Axum) backend in `apps/backend`. See `ARCHITECTURE.md`.
- Backend structure: keep the layering `handlers -> services -> repositories` (handlers parse/extract only; business logic stays in services). Entry: `apps/backend/src/main.rs`.
- Frontend API calls: prefer shared helpers in `apps/web/src/lib/api.ts` (includes retry/backoff behavior for cold starts).
- Local dev: `pnpm --filter web dev` (frontend) and `cargo run` (backend), with `NEXT_PUBLIC_API_URL` pointing at the backend.
