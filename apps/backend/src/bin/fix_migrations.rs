...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = sqlx::query("ALTER TABLE session_scores DROP FOREIGN KEY fk_2")
        .execute(&pool)
        .await;
    let _ = sqlx::query("ALTER TABLE session_scores DROP FOREIGN KEY fk_3")
        .execute(&pool)
        .await;
    ...
    sqlx::query("DROP TABLE IF EXISTS interactions")
        .execute(&pool)
        .await?;
    sqlx::query("DROP TABLE IF EXISTS questions")
        .execute(&pool)
        .await?;
    sqlx::query("DROP TABLE IF EXISTS votes")
        .execute(&pool)
        .await?;
    sqlx::query("DROP TABLE IF EXISTS participants")
        .execute(&pool)
        .await?;
